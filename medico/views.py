...
def consulta_area_medico(request, id_consulta):
    ...
    if request.method == "GET":
        ...
        return render(request, 'consulta_area_medico.html', {'consulta': consulta,'is_medico': is_medico(request.user),'documentos':documento}) 
    elif request.method == "POST":
...
def finalizar_consulta(request, id_consulta):
    ...
    if request.user != consulta.data_aberta.user:
        messages.add_message(request, constants.ERROR, 'Esta consulta não pertence ao seu usuario.')
        return redirect(f'/medicos/abrir_horario/')
    
    consulta.status= 'F'
    consulta.save()
    return redirect(f'/medicos/consulta_area_medico/{id_consulta}')
#TODO:fazer botao para o paciente cancelar a consulta que é parecido com o finalizar do medico
